    ...
    public partial class Administrador : Form
    {
        ...
        private async Task<bool> LogoutAPI()
        {
          string baseUrl = PantallaPrincipal.apiBase.Replace("/auth", "");
            ...
            string url = $"{baseUrl}/admin/students?sessionId={PantallaPrincipal.SessionId}";
            try
            {
        ...
        private void Alumnes_Click_1(object sender, EventArgs e)
        {
            Alumnes form = new Alumnes();
            form.Show();
        }


        private void Empreses_Click(object sender, EventArgs e)
        {
            Empreses form = new Empreses();
            form.Show();
        }
    }
}